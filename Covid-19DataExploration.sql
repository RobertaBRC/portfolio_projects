...
-------------------------------------------------------------------------------------------------------------------- 
-- Examples of view to be created for reporting purposes (using COALESCE to replace 'NULL' with 0):
...
SELECT 
	...
	COALESCE(ROUND(SUM(CONVERT(NUMERIC,v.people_fully_vaccinated) / population) * 100, 4), 0) AS perc_fully_vaccinated
FROM PortfolioProject..CovidDeaths AS d
JOIN PortfolioProject..CovidVaccinations AS v
	ON d.location = v.location
	AND d.date = v.date
...
SELECT 
	...
	COALESCE(ROUND(SUM(CONVERT(INT, new_deaths)) / SUM(new_cases) * 100, 4), 0) AS fatality_rate,
	CASE 
		WHEN  SUM(CONVERT(INT, new_deaths)) / SUM(new_cases) * 100 < 2 THEN 'Low'
		WHEN  SUM(CONVERT(INT, new_deaths)) / SUM(new_cases) * 100 >= 2 AND 
		      SUM(CONVERT(INT, new_deaths)) / SUM(new_cases) * 100 <= 5 THEN 'Medium'
